...
module.exports = new RpcWorker('personas', class extends RpcProvider {
  ...
  async create(profile) {
    ...
    try {
      const personas = db.collection(PERSONA_COLLECTION_NAME)
      await personas.insertOne(profile, function(err, docInserted){
      })
    } catch (err) {
  ...
  async show(id) {
    ...
    const result_count = await searchCursor.count()

    console.log("result count: ", result_count)
    const persona_found = await searchCursor.toArray()
    console.log("persona found:", persona_found)
  }

  async edit(id, updated_profile) {
    // TODO
    return 0
  }
