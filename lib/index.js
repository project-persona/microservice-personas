...
module.exports = new RpcWorker('personas', class extends RpcProvider {
  ...
  async [RpcProvider.before] () {
    ...
    this.user = await admin.auth().verifyIdToken(this.context.authorization)
  }
  ...
  async create (persona) {
    const { alias, lastName, firstName, age, birthday, gender, email, address, phoneNumber } = persona
    const payload = {
      alias,
      lastName,
      firstName,
  ...
  async edit (id, persona) {
    const acceptableKeys = ['alias', 'lastName', 'firstName', 'age', 'birthday', 'gender', 'address', 'phoneNumber']
